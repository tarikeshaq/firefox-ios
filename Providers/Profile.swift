...
open class BrowserProfile: Profile {
     ...
     */
    init(localName: String, syncDelegate: SyncDelegate? = nil, clear: Bool = false, historyApiConfiguration: HistoryAPIConfiguration = .old) {
        ...
        self.historyApiConfiguration = historyApiConfiguration
...
//            authState?.clientName = account.deviceName
        ...
        @discardableResult public func syncEverything(why: SyncReason) -> Success {
            ...
            var historySynchronizer = ("history", self.syncLegacyHistoryWithDelegate)
            if self.profile.historyApiConfiguration == .new {
                historySynchronizer = ("history", self.syncHistoryWithDelegate)
            }
            ...
            let synchronizers = [
                ...
                ("history", self.syncLegacyHistoryWithDelegate),
                historySynchronizer,
    ...
    public func setHistoryConfigurationAPI(historyApiConfiguration: HistoryAPIConfiguration) {
        self.historyApiConfiguration = historyApiConfiguration
    }
}
