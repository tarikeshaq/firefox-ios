...
class HistoryPanelViewModel: Loggable, FeatureFlaggable {
        ...
        case .new:
            ...
            profile.places.queryAutocomplete(matchingSearchQuery: term, limit: searchQueryFetchLimit).uponQueue(.main) { result in
                ...
                guard result.isSuccess else {
                    SentryIntegration.shared.sendWithStacktrace(message: result.failureValue?.localizedDescription ?? "Unkown error searching history", tag: .rustPlaces, severity: .error)
                    completion(false)
                    return
                }
    ...
    private func fetchData() -> Deferred<Maybe<Cursor<Site>>> {
        ...
        switch profile.historyApiConfiguration {
            ...
            case .new:
            return profile.places.getVisitPageWithBound(limit: queryFetchLimit, offset: currentFetchOffset, excludedTypes: VisitTransitionSet(0)) >>== { result in
                DispatchQueue.main.asyncAfter(deadline: .now() + .milliseconds(100)) {
                    self.isFetchInProgress = false
                    self.browserLog.debug("currentFetchOffset is: \(self.currentFetchOffset)")
                }
